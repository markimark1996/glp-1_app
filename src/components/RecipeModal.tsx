...
export function RecipeModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-full sm:max-w-6xl h-screen sm:h-auto max-h-screen p-0 gap-0 border-none bg-white sm:max-h-[95vh] sm:rounded-lg relative flex flex-col">
        ...
        {/* Scrollable Content */}
        <div className="overflow-y-auto flex-1" style={{ paddingBottom: '80px' }}>
          {/* Hero Image Section */}
        ...
        <div className="absolute bottom-0 left-0 right-0 sm:relative border-t border-gray-200 px-4 sm:px-8 py-4 sm:py-6 bg-white z-10" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}>
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            ...
            >
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="hidden sm:inline">{isAddedToMealPlan ? 'Added to Meal Plan' : 'Add to Meal Plan'}</span>
                <span className="sm:hidden text-xs">Add to Plan</span>
              </div>
            </button>
            <button
              onClick={handleToggleFavorite}
              className={`py-3 sm:py-4 px-2 sm:px-6 transition-all font-medium text-sm sm:text-base whitespace-nowrap ${
                isFavorite
            ...
            >
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="hidden sm:inline">{isFavorite ? 'Recipe Saved' : 'Save Recipe'}</span>
                <span className="sm:hidden text-xs">{isFavorite ? 'Saved' : 'Save'}</span>
              </div>
            </button>
          </div>
        </div>
